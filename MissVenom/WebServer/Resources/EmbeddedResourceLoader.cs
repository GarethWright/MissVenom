...
namespace HttpServer.Resources
{
    ...
    {
        private readonly ILogger _logger = LogFactory.CreateLogger(typeof (EmbeddedResourceLoader));
        ...
        public void Add(string path, Assembly assembly, string rootNameSpace, string fullResourceName)
        {
            ...
            _logger.Trace("Adding mapping '" + path + filePath + "' to resource '" + fullResourceName + "' assembly '" +
                          assembly +
                          "'.");
        ...
        public bool AddFile(string path, Assembly assembly, string resourceName)
        {
			...
			_logger.Trace("Added " + path + " = " + resourceName);
    	...
    	private void LoadFromPathMappings(string path)
    	{
			...
			foreach (var mapping in _pathMappings)
    		{
    			...
    			if (AddFilesInFolder(path + "/", mapping.Assembly, myNamespace.ToLower()))
	    			_logger.Debug("Added all files in namespace '" + myNamespace + "' from uri '" + path + "'.");
    		}
    	}

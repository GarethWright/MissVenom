...
namespace HttpServer.Routing
{
    ...
    public class SimpleRouter : IRouter
    {
        ...
        private readonly ILogger _logger = LogFactory.CreateLogger(typeof (SimpleRouter));
        ...
        public virtual ProcessingResult Process(RequestContext context)
        {
            ...
            if (request.Uri.AbsolutePath == FromUrl)
            {
                if (!ShouldRedirect)
                {
                    _logger.Debug("Redirecting (internally) from " + FromUrl + " to " + ToUrl);
                    request.Uri = new Uri(request.Uri, ToUrl);
                    return ProcessingResult.Continue;
                }

                _logger.Debug("Redirecting browser from " + FromUrl + " to " + ToUrl);
